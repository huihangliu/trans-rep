...
parser.add_argument("--T", help="number of source dataset", type=int, default=3)
...
def joint_train(model_names):
  ...
  if 'mtl-pl-2lr' in model_names:
    ...
    test_loss = evaluate(models['mtl-pl-2lr']['target-nn'], dgps.dataloader_target_test, criterion)
    test_perf['mtl-pl-2lr'] = test_loss
    beta_hat = models['mtl-pl-2lr']['target-nn'].linear_output.weight.detach().numpy()[0, :dgps.dim_linear]
  ...
  J_true = X_train.T @ (np.eye(dgps.n_train_target) - H_true @ np.linalg.inv(H_true.T @ H_true) @ H_true.T) @ X_train / dgps.n_train_target
  tmp = np.eye(dgps.n_train_target) - H_true @ np.linalg.inv(H_true.T @ H_true) @ H_true.T
  beta_hat_oracle =  np.linalg.inv(X_train.T @ tmp @ X_train) @ X_train.T @ tmp @ dgps.data_target['label_train']
  tmp = np.eye(dgps.n_train_target) - X_train @ np.linalg.inv(X_train.T @ X_train) @ X_train.T
  gamma_hat_oracle = np.linalg.inv(H_true.T @ tmp @ H_true) @ H_true.T @ tmp @ dgps.data_target['label_train']
  sigma2_hat = np.linalg.norm(dgps.data_target['label_train'] - X_train @ beta_hat_oracle - H_true @ gamma_hat_oracle)**2 / dgps.n_train_target
  est_var_oracle = sigma2_hat * np.linalg.inv(J_true) / dgps.n_train_target
  print(f"Known h(z) : sigma2_hat: {sigma2_hat}")
  print(f"shape of Y: {dgps.data_target['label_train'].shape}, shape of X: {X_train.shape}, shape of H: {H_true.shape}, shape of beta_hat: {beta_hat_oracle.shape}, shape of gamma_hat: {gamma_hat_oracle.shape}")
  # [end] debug
  ...
  return pred_result, est_result, beta_hat, est_var, beta_hat_oracle, est_var_oracle
...
dgps = DGPs(T=args.T, dim_linear=args.q, dim_nonlinear=args.p, dim_rep=args.r, rep_flag=rep_flag, coef_flag=coef_flag, feature_flag=feature_flag, n_train_source=args.n_source, n_train_target=args.n_target, noise_level=noise_level)

if args.r == 2:
  dgps.params['func_idx'] = [5, 6] # depth 2 width 64 T 2 r 2
if args.r == 3:
  dgps.params['func_idx'] = [2, 3, 4] # depth 3 width 300 T 5 r 3
  # dgps.params['func_idx'] = [2, 5, 6] # depth 3 width 300 T 5 r 3
if args.r == 5:
  dgps.params['func_idx'] = [2, 3, 4, 5, 6]
...
test_l2_error, est_error, beta_hat, var_hat, beta_hat_ols, var_hat_ols = joint_train(model_names)
...
tmp_res = np.array([beta_hat, var_hat[0, :], beta_hat_ols[0, :], var_hat_ols[0, :]]).reshape((1, -1))
if args.verbose: print(f"beta_hat: {beta_hat}, var_hat: {var_hat}")
...
if False:
  ...
  # add ledgend: the dashed line is the true representation, the solid line is the predicted representation
  plt.legend([f"Est. rep. {i}" for i in range(args.r)], loc='upper left')
