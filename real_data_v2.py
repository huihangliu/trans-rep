...
note:
  Real data
  1. 标准化, y 需要中心化
  2. 需要构造 x, 所有的数据都与对照组的数据进行比较, 否则无法建立回归模型. 这样, 数据就只有 6 组了. 表征的维度至多是 6 组. 
"""
...
def pairwise_distances(x, y=None): # Function for computing pairwise distances between vectors in x and y
...
def joint_train(model_names):
  ...
  est_perf = {}
  pred_perf = {}
  best_shared_layers = SharedNN(dim_input=p, dim_rep=r, width=width, depth=depth, flag_batchnorm=False)
  ...
  if 'ols-linear' in model_names:
    ...
    est_beta = np.linalg.lstsq(X_train_linear, dgps.data_target['label_train'], rcond=None)[0][:,0]
    est = est_beta
    est_perf['ols-linear'] = est
    pred_loss = np.mean((dgps.data_target['label_test'] - dgps.data_target['feature_test_spline'] @ est_beta)**2)
    pred_perf['ols-linear'] = pred_loss

  if 'trans-lasso-spline' in model_names:
    if args.verbose: print(f"------------ Trans Lasso Spline -----------\n")
    ...
    pred_loss = np.mean((dgps.data_target['label_test'] - dgps.data_target['feature_test_spline'] @ est_beta_gamma)**2)
    pred_perf['trans-lasso-spline'] = pred_loss
    
    est = est_beta_gamma[:dgps.dim_linear]
    est_perf['trans-lasso-spline'] = est

    loss_valid = np.mean((dgps.data_target['label_valid'] - dgps.data_target['feature_valid_spline'] @ est_beta_gamma)**2)
    if args.verbose: print(f"Valid loss: {loss_valid}, Test loss: {pred_loss}")

  if 'meta-analysis-spline' in model_names:
    if args.verbose: print(f"----------- Meta Analysis Spline ----------\n")
    model_averaging = ModelAveraging(num_models=T+1)
    X_list = [dgps.data_target['feature_train_spline']] + [dgps.data_source['feature_train_spline'][t] for t in range(T)]
    Z_list = [dgps.data_target['feature_train_spline'][:, :0]] + [dgps.data_source['feature_train_spline'][t][:, :0] for t in range(T)]
    Y_list = [dgps.data_target['label_train']] + dgps.data_source['label_train']
  ...
  if 'pooled-regression-spline' in model_names:
    if args.verbose: print(f"--------- Pooled Regression Spline --------\n")
    model_averaging = ModelAveraging(num_models=T+1)
    X_list = [dgps.data_target['feature_train_spline']] + [dgps.data_source['feature_train_spline'][t] for t in range(T)]
    Z_list = [dgps.data_target['feature_train_spline'][:, :0]] + [dgps.data_source['feature_train_spline'][t][:, :0] for t in range(T)]
    Y_list = [dgps.data_target['label_train']] + dgps.data_source['label_train']
  ...
  if 'map-spline' in model_names:
    if args.verbose: print(f"----------------- MAP Spline --------------\n")
    model_averaging = ModelAveraging(num_models=T+1)
    X_list = [dgps.data_target['feature_train_spline']] + [dgps.data_source['feature_train_spline'][t] for t in range(T)]
    Z_list = [dgps.data_target['feature_train_spline'][:, :0]] + [dgps.data_source['feature_train_spline'][t][:, :0] for t in range(T)]
    Y_list = [dgps.data_target['label_train']] + dgps.data_source['label_train']
  ...
  if 'map' in model_names:
    if args.verbose: print(f"----------------- MAP Spline --------------\n")
    model_averaging = ModelAveraging(num_models=T+1)
    X_list = [dgps.data_target['feature_train']] + [dgps.data_source['feature_train'][t] for t in range(T)]
    Z_list = [dgps.data_target['feature_train'][:, :0]] + [dgps.data_source['feature_train'][t][:, :0] for t in range(T)]
    Y_list = [dgps.data_target['label_train']] + dgps.data_source['label_train']
    res_map = model_averaging.map(Y_list, X_list, Z_list)
    est_beta_gamma = np.concatenate((res_map['beta_hat'], res_map['alpha_hat']))
    ...
    est = res_map['beta_hat'][:dgps.dim_linear]
    est_perf['map'] = est
    pred_loss = np.mean((dgps.data_target['label_test'] - dgps.data_target['feature_test'] @ est_beta_gamma)**2)
    pred_perf['map'] = pred_loss

    loss_valid = np.mean((dgps.data_target['label_valid'] - dgps.data_target['feature_valid'] @ est_beta_gamma)**2)
    if args.verbose: print(f"Valid loss: {loss_valid}, Test loss: {pred_loss}")

  if 'mtl-pl-2lr' in model_names:
    flag_aux_train = 'default' # 'default', 'joint', 'orthogonal'
    if args.verbose: print(f"------------------ MTL[Our] ---------------\n")
    ...
    for epoch in range(num_epochs_pretrain):
      ...
      if epoch % 40 == 0 and args.verbose: print(f"Epoch {epoch}\n--------------------")
      ...
      if flag_aux_train == 'default': # source only
        ...
        valid_loss = evaluate([models['mtl-pl-2lr']['target-nn']] + models['mtl-pl-2lr']['aux-nn'], dgps.dataloaders_source_valid+[dgps.dataloader_target_valid], criterion)
      elif flag_aux_train == 'orthogonal':
        if epoch < 10: zlr = 6.0
        elif epoch == 20: zlr = 3.0
      ...
      if epoch % 40 == 0 and args.verbose: print(f"train loss: {train_loss}, valid loss: {valid_loss}")
      last_update_count += 1
      if np.sum(valid_loss) < best_valid['mtl-pl-2lr']:
        ...
        if True and args.verbose: print(model_color['mtl-pl-2lr'] + f"Model [{'mtl-pl-2lr'}] update best valid loss {valid_loss}"
        )
        last_update_count = 0
      if last_update_count > patience: #  and max(valid_loss) < 5
        # early stop
        break
    if args.verbose: print(f"--------------------End shared layers--------------------\n")
    ...
    if flag_test: test_loss = evaluate(models['mtl-pl-2lr']['target-nn'], dgps.dataloader_target_test, criterion)
    est = models['mtl-pl-2lr']['target-nn'].linear_output.weight.detach().numpy()[0, :dgps.dim_linear]
    est_perf['mtl-pl-2lr'] = est
    print(f"estimated beta: {est}")
    print(f"estimates of coeff: {models['mtl-pl-2lr']['target-nn'].linear_output.weight.detach().numpy()[0, :]}")
    if flag_test: pred_perf['mtl-pl-2lr'] = test_loss
    ...
    for t in range(T):
      ...
      res_mtl.append(models['mtl-pl-2lr']['aux-nn'][t].linear_output.weight.detach().numpy()[0, :dgps.dim_linear])

    # estimate the noise level
    sigma2_hat = train_loss_target.detach().numpy() # this is the mean of prediction error on the target training data
    ...
    mu_hat = X_train.T @ H_hat @ np.linalg.inv(H_hat.T @ H_hat + np.eye(args.r)*1e-8)
    XH_trin = np.concatenate((X_train, H_hat), axis=1)
    est_coef = np.linalg.inv(XH_trin.T @ XH_trin) @ XH_trin.T @ dgps.data_target['label_train']
    ...
    for i in range(dgps.n_train_target):
      tmp += residual[i]**2 * (X_train[i, :].T - mu_hat @ H_hat[i, :].T) @ (X_train[i, :].T - mu_hat @ H_hat[i, :].T).T
    tmp = tmp / dgps.n_train_target
    J_hat = X_train.T @ (np.eye(dgps.n_train_target) - H_hat @ np.linalg.inv(H_hat.T @ H_hat + np.eye(args.r)*1e-8) @ H_hat.T) @ X_train / dgps.n_train_target + np.eye(dgps.dim_linear)*1e-8
    if False: # old version
      est_var = sigma2_hat * np.linalg.inv(J_hat + np.eye(dgps.dim_linear)*1e-8) / dgps.n_train_target # the estimated variance of the estimator with known noise level
    else:
      est_var = np.linalg.inv(J_hat) @ tmp @ np.linalg.inv(J_hat) / dgps.n_train_target
    print(f"Estimated variance of the estimator: {est_var}")
    print(f"old version: {sigma2_hat * np.linalg.inv(J_hat) / dgps.n_train_target}")
    # [end] estimate variance

    if args.verbose: print(f"--------------------End target model--------------------\n")
  ...
  for epoch in range(num_epochs):
    ...
    for model_name in model_names:
      ...
      if valid_loss < best_valid[model_name]:
        ...
        est = models[model_name].linear_output.weight.detach().numpy()[0, :dgps.dim_linear] if model_name != 'vanilla-orginal' else 0
        est_perf[model_name] = est
        if args.verbose: print(model_color[model_name] + f"Model [{model_name}]: update current train loss: {train_loss}, best valid loss: {valid_loss}")
      if epoch % 40 == 0:
        if args.verbose: print(Fore.BLACK + f"Model [{model_name}]: Epoch {epoch}/{num_epochs}, Train MSE: {train_loss}, Valid MSE: {valid_loss}")
      pred_loss = evaluate(models[model_name], dgps.dataloader_target_test, criterion)
  ...
  for i, name in enumerate(model_names):
    ...
    if flag_test: pred_result[0, i] = pred_perf[name]

  est_result = est_perf['mtl-pl-2lr'].reshape(1, -1) # only return the estimates of mtl method
...
vanilla_nn_model = PartialRegressionNN(dim_input=p+q, dim_linear=q, depth=depth, width=width, bias_last_layer=True)
dropout_nn_model = PartialRegressionNN(dim_input=p+q, dim_linear=q, depth=depth, width=width, input_dropout=True, dropout_rate=0.6, bias_last_layer=True)
dropout_my_nn_model = PartialLinearRegression(dim_input=p+q, dim_linear=q, dim_rep=r, depth=depth, width=width, input_dropout=True, dropout_rate=0.6)
shared_nn = SharedNN(dim_input=p, dim_rep=r, width=width, depth=depth, flag_batchnorm=False)
nn_models_aux = [PartialLinearRegression(dim_linear=q, shared_layers=shared_nn, flag_binary_outcome=flag_binary_outcome) for t in range(T)]
nn_model_target = PartialLinearRegression(dim_linear=q, shared_layers=shared_nn, flag_binary_outcome=flag_binary_outcome)
stl_1lr_model = PartialLinearRegression(dim_input=p+q, dim_linear=q, depth=depth, width=width)
dropout_stl_1lr_model = PartialLinearRegression(dim_input=p+q, dim_linear=q, depth=depth, width=width, input_dropout=True, dropout_rate=0.6)
stl_2lr_model = PartialLinearRegression(dim_input=p+q, dim_linear=q, dim_rep=r, depth=depth, width=width, flag_batchnorm=False)
vanilla_pl_1lr_model = PartialRegressionNN(dim_input=p+q, dim_linear=q, depth=depth, width=width, bias_last_layer=True)
dropout_vanilla_pl_1lr_model = PartialRegressionNN(dim_input=p+q, dim_linear=q, depth=depth, width=width, input_dropout=True, dropout_rate=0.6, bias_last_layer=True)
...
if flag_test == False: model_names = ['mtl-pl-2lr']
...
if len(args.record_dir) > 0:
  ...
  res_pred_file.close()
end_time = time.time()
print(f"Case with exp_id {args.exp_id}, T = {dgps.T}, r = {args.r}, seed = {args.seed} done: time = {end_time - start_time} secs")
if args.verbose: print(f"Est: {est}\nPredition Loss: {pred_perf}")
