...
class HidenModel:
  ...
  def __init__(self, num_funcs=3, flag_linear=False):
    ...
    self.num_funcs = num_funcs
    self.func_idx = self.func_idx = np.random.randint(1, 6, self.num_funcs)   # exclude the first function x
    # # # The following is Fan's setting
    # np.random.randint(1, len(self.func_zoo), self.num_funcs)
    if num_funcs > 0: self.func_idx[0] = 5
    # self.func_idx[1] = 2
    # self.func_idx[2] = 1
    # self.func_idx[3] = 2
    if num_funcs > 4: self.func_idx[4] = 5
...
class NonlinearRegressionModel:
  ...
  def __init__(self, num_funcs=3, normalize=False, hiden_feature=None):
    ...
    if num_funcs != hiden_feature.num_funcs:
      ...
      Warning("RegressionModel_Huang: num_funcs doesn't match! Use the param in hiden_feature")
...
class PartialLinearModel:
  '''
    ...
    sample(x, z)
      ...
      Return function value given the covariate
  '''
  def __init__(self, dim_input, dim_linear=0, dim_nonlinear=None, normalize=False, hiden_feature=None):
    '''
      A function to initialize the additive model used
    ...
    self.dim_nonlinear = dim_input - dim_linear if dim_nonlinear is None else dim_nonlinear # if nonlinear dim is not specified, use ALL rest of the input expect linear part

    self.linear_coef = np.random.randn(self.dim_linear, 1)
    self.gamma = np.random.randn(self.dim_nonlinear, 1) #  np.ones((self.dim_nonlinear, 1))
    ...
    if self.dim_nonlinear != hiden_feature.num_funcs:
      ...
      Warning("PartialLinearModel: num_funcs doesn't match! Use the param in hiden_feature")
  ...
  def sample(self, x):
    ...
    if np.shape(feature_nonlinear)[1] < self.dim_nonlinear:
      raise ValueError("PartialLinearModel: Nonlinear part data dimension {}, ".format(np.shape(feature_nonlinear)[1]) +
    ...
    for i in range(self.dim_nonlinear):
      y += self.func_zoo[self.func_idx[i]](feature_nonlinear[:, i:i + 1]) * self.gamma[i] # fixed a bug: x -> feature_nonlinear [2023-12-11]
  ...
  def __str__(self):
    ...
    s += "\tNonlinear Rep: h = \n"
    for i in range(self.dim_nonlinear):
      s = s + f"\t   {self.hiden_feature.func_name[self.hiden_feature.func_idx[i]]} (z_{i + 1})\t"
    return s
...
class DGPs:
  ...
  """
  def __init__(self, T=None, dim_linear=None, dim_nonlinear=None, dim_rep=None, rep_flag=None, coef_flag=None, feature_flag=None, n_train_target=None, n_train_source=None, batch_size=None, noise_level=None, scale=None, profile=None, n_knots=None, degree=None, verbose=False, seed=None):
    """
    parameters:
    ...
    self.batch_size = batch_size if batch_size is not None else self.n_train_source # 128 self.n_train_source
    ...
    if self.rep_flag == 'default' or self.rep_flag == 'linear':
      ...
      if self.dim_rep is None: ValueError("Please specify the dimension of representation for linear-factor design!")
      ...
      if self.dim_rep is None: ValueError("Please specify the dimension of representation for additive-factor design!")
      ...
      self.params['A'] = np.random.randn(self.dim_nonlinear, self.dim_rep) / np.sqrt(self.dim_nonlinear) 
    ...
    if self.coef_flag == 'default' or self.coef_flag == 'homogeneous':
      if False:
        self.params['betas'] = [np.random.uniform(-1, 1, size=(self.dim_linear, 1))] * T
        self.params['gammas'] = [np.random.uniform(-1, 1, size=(self.dim_rep, 1))] * T
      else:
      ...
      if False: # use uniform distribution
        self.params['betas'] = [np.random.uniform(-1, 1, size=(self.dim_linear, 1)) for t in range(T)]
        self.params['gammas'] = [np.random.uniform(-1, 1, size=(self.dim_rep, 1)) for t in range(T)]
        self.params['beta'] = np.random.uniform(-1, 1, size=(self.dim_linear, 1))
        self.params['gamma'] = np.random.uniform(-1, 1, size=(self.dim_rep, 1))
      else: # use normal distribution
        self.params['betas'] = [np.random.randn(self.dim_linear, 1) for t in range(T)]
        self.params['gammas'] = [np.random.randn(self.dim_rep, 1) * 1 for t in range(T)]
      ...
      self.params['gamma'] = np.concatenate(([0.3]*self.sparsity_gamma, [0]*(self.dim_nonlinear-self.sparsity_gamma)), axis=0).reshape(-1, 1)
    ...
    if self.scale:
      self.params['func_scale'] = 2.0 / np.sqrt(np.array([0.3333, 0.2727, 0.3671, 0.0889, 0.0189, 0.5000, 0.5000, 0.1987, 0.0193, 0.5876, 0.1905, 1.2792, 0.0889, 0.2453])) # scale the function to have unit variance
    else:
    ...
    if True:
      ...
      if self.rep_flag == 'additive':
        ...
        if self.dim_rep >= 4: self.params['func_idx'][4] = 4
    else:
    ...
    if self.rep_flag == 'additive':
      # scale the function to have unit variance
      for i in range(self.dim_rep):
        self.params['gammas'][i] = self.params['gammas'][i] * self.params['func_scale'][self.params['func_idx']].reshape(-1, 1)
      # self.params['gamma'] = self.params['gammas'][0]
      # self.params['beta'] = self.params['betas'][0]
    else:
  ...
  def h(self, z):
    ...
    if self.rep_flag == 'additive':
      ...
      for i in range(self.dim_rep):
        output[:, i:i+1] = self.func_zoo[self.params['func_idx'][i]](z[:, i:i+1])
      return output
    elif self.rep_flag == 'additive-factor':
      ...
      for i in range(self.dim_rep):
        output[:, i:i+1] = self.func_zoo[self.params['func_idx'][i]](tmp_z[:, i:i+1])
      return output
    elif self.rep_flag == 'deep':
  ...
  def tune_knots(self):
    ...
    for n_knots in n_knots_list:
      ...
      lm = LinearRegression(fit_intercept=False)# RidgeCV(cv=5, fit_intercept=False)
      lm.fit(X, Y)
      tmp_valid_loss = np.mean((lm.predict(self.data_target['feature_valid_spline']) - self.data_target['label_valid'])**2)
  ...
  def sample(self, n_knots=11, seed=2024):
    ...
    common_knots = np.concatenate(([inner_knots[0]]*degree, inner_knots, [inner_knots[-1]]*degree))
    ...
    for t in range(self.T):
      if self.feature_flag == 'default' or self.feature_flag == 'iid-normal':
        all_feature = np.random.randn(self.n_train_source+self.n_valid_source+self.n_test_source, self.dim_linear+self.dim_nonlinear)
      elif self.feature_flag == 'iid-uniform':
        all_feature = np.random.uniform(-1, 1, (self.n_train_source+self.n_valid_source+self.n_test_source, self.dim_linear+self.dim_nonlinear))
        # uniform range 1, 1.5
      elif self.feature_flag == 'x_uniform-z_factor':
        # unfinished case
        x = np.random.uniform(-1, 1, (self.n_train_source+self.n_valid_source+self.n_test_source, self.dim_linear))
        z = np.random.randn(self.n_train_source+self.n_valid_source+self.n_test_source, self.dim_nonlinear)
        ...
        cov = np.ones((self.dim_linear+self.dim_nonlinear, self.dim_linear+self.dim_nonlinear)) * 0.5
        # set the diagonal of cov to be 1
        row, col = np.diag_indices_from(cov)
        cov[row, col] = 1
        all_feature = np.random.multivariate_normal(mean_value, cov, (self.n_train_source+self.n_valid_source+self.n_test_source))
      else:
      ...
      y = y_true + np.random.randn(self.n_train_source+self.n_valid_source+self.n_test_source, 1) * self.noise_level

      all_feature_train = all_feature[:self.n_train_source, :]
      y_train = y[:self.n_train_source, :]
      all_feature_valid = all_feature[self.n_train_source:self.n_train_source+self.n_valid_source, :]
      y_valid = y[self.n_train_source:self.n_train_source+self.n_valid_source, :]
      all_feature_test = all_feature[self.n_train_source+self.n_valid_source:, :]
      ...
      dataloader_train = DataLoader(RegressionDataset(all_feature_train, y_train), batch_size=self.batch_size, shuffle=True)
      dataloader_valid = DataLoader(RegressionDataset(all_feature_valid, y_valid), batch_size=self.batch_size, shuffle=True)
      dataloader_test = DataLoader(RegressionDataset(all_feature_test, y_test), batch_size=self.batch_size, shuffle=False)
      ...
      self.data_source['feature_train_spline'].append(all_feature_spline[:self.n_train_source, :]) 

    # generate target data
    if self.feature_flag == 'default' or self.feature_flag == 'iid-normal':
      all_feature = np.random.randn(self.n_train_target+self.n_valid_target+self.n_test_target, self.dim_linear+self.dim_nonlinear)
    elif self.feature_flag == 'iid-uniform':
      all_feature = np.random.uniform(min_val, max_val, (self.n_train_target+self.n_valid_target+self.n_test_target, self.dim_linear+self.dim_nonlinear))
    elif self.feature_flag == 'x_uniform-z_factor':
      x = np.random.uniform(-1, 1, (self.n_train_target+self.n_valid_target+self.n_test_target, self.dim_linear))
      z = np.random.randn(self.n_train_target+self.n_valid_target+self.n_test_target, self.dim_nonlinear)
      all_feature = np.concatenate((x, z), axis=1)
    elif self.feature_flag == 'compound-normal':
      mean_value = np.zeros(self.dim_linear+self.dim_nonlinear)
      cov = np.ones((self.dim_linear+self.dim_nonlinear, self.dim_linear+self.dim_nonlinear)) * 0.8
      # set the diagonal of cov to be 1
      row, col = np.diag_indices_from(cov)
      cov[row, col] = 1
      all_feature = np.random.multivariate_normal(mean_value, cov, (self.n_train_target+self.n_valid_target+self.n_test_target))
    else:
      raise ValueError("Unknown feature_flag: {}".format(self.feature_flag))
    ...
    y = y_true + np.random.randn(self.n_train_target+self.n_valid_target+self.n_test_target, 1) * self.noise_level
    R2_X = np.var(x @ self.params['beta']) / np.var(y)
    R2_eps = np.var(y_true) / np.var(y)
    R2_h = np.var(self.h(z) @ self.params['gamma']) / np.var(y)
    if self.verbose: print(f"[DGP] R2 of target model linear part: {R2_X}")
    if self.verbose: print(f"[DGP] R2 of target model non-linear part: {R2_h}")
    if self.verbose: print(f"[DGP] R2 of target model: {R2_eps}")
    # end of calculating R-square

    # data perprocessing
    scaler = StandardScaler()
    all_feature_scaled = np.concatenate((all_feature[:, :self.dim_linear], scaler.fit_transform(all_feature[:, self.dim_linear:])), axis=1)
    # end of data preprocessing

    dataloader_train = DataLoader(RegressionDataset(all_feature_scaled[:self.n_train_target, :], y[:self.n_train_target, :]), batch_size=self.batch_size, shuffle=True)
    dataloader_valid = DataLoader(RegressionDataset(all_feature_scaled[self.n_train_target:self.n_train_target+self.n_valid_target, :], y[self.n_train_target:self.n_train_target+self.n_valid_target, :]), batch_size=self.batch_size, shuffle=True)
    dataloader_test = DataLoader(RegressionDataset(all_feature_scaled[self.n_train_target+self.n_valid_target:, :], y_true[self.n_train_target+self.n_valid_target:, :]), batch_size=self.batch_size, shuffle=True)
    ...
    if True:
      ...
      all_feature_spline = np.zeros((self.n_train_target+self.n_valid_target+self.n_test_target, self.dim_linear+self.dim_nonlinear))
    ...
    self.data_target = {
      ...
      'feature_valid': all_feature[self.n_train_target:self.n_train_target+self.n_valid_target, :],
      'feature_valid_spline': all_feature_spline[self.n_train_target:(self.n_train_target+self.n_valid_target), :],
      'label_valid': y[self.n_train_target:(self.n_train_target+self.n_valid_target), :],
      'feature_test': all_feature[self.n_train_target+self.n_valid_target:, :],
      'feature_test_spline': all_feature_spline[(self.n_train_target+self.n_valid_target):, :],
...
class MyDeepModel(torch.nn.Module):
  ...
  def __init__(self):
    ...
    def func9(x):
      x = torch.where(x > 1., torch.tensor(1.), x)
      x = torch.where(x < -1., torch.tensor(-1.), x)
    ...
    def func12(x):
      x = torch.where(x > 2., torch.tensor(2.), x)
      return x**2
  ...
  def forward(self, z):
    ...
    z1, z2, z3, z4, z5, z6, z7, z8, z9, z10 = z[:, 0:1], z[:, 1:2], z[:, 2:3], z[:, 3:4], z[:, 4:5], z[:, 5:6], z[:, 6:7], z[:, 7:8], z[:, 8:9], z[:, 9:10]
    ...
    g6_output =  self.func_zoo[self.func_idx[5]](self.g6(torch.cat((z9, z10), dim=1)))

    # Process g outputs through h layers
    h1_output = self.func_zoo[self.func_idx[6]](self.h1(torch.cat((g1_output, g2_output), dim=1)))
    h2_output = self.func_zoo[self.func_idx[7]](self.h2(torch.cat((g2_output, g3_output), dim=1)))
    h3_output = self.func_zoo[self.func_idx[8]](self.h3(torch.cat((g3_output, g4_output), dim=1)))
    h4_output = self.func_zoo[self.func_idx[9]](self.h4(torch.cat((g4_output, g5_output), dim=1)))
    h5_output = self.func_zoo[self.func_idx[10]](self.h5(g6_output))

    # Return the final output
    return torch.cat((h1_output, h2_output, h3_output, h4_output, h5_output), axis=1)
...
class RealData:
  ...
  def __init__(self, data_name=None, flag_test=False, verbose=False):
    ...
    if self.data_name == 'penn_jae': 
      ...
      self.dim_linear = None
      self.dim_nonlinear = None
      self.T = None  # total 5 datasets, 1 target and 4 source
      self.fetch_house()
      if flag_test: self.tune_knots()
    else:
  ...
  def fetch_bouns(self, idx_target = 4):
    raw_data = pd.read_csv('/Users/huihang/Library/CloudStorage/OneDrive-Personal/Repository/Generative_model/CGAN/CGAN-python/real_data/penn_jae/penn_jae.dat', delim_whitespace=True)
    ...
    data['inuidur1'] = data.groupby('tg', group_keys=False)['inuidur1'].apply(lambda x: x - np.mean(x))
    ...
    d_cols = ['tg']
    x_cols = ['female']
    z_cols = ['black', 'othrace', 'dep1', 'dep2', 'q2', 'q3', 'q4', 'q5', 'q6', 'agelt35', 'agegt54', 'durable', 'lusd', 'husd']
    ...
    self.dataloader_target_train = DataLoader(RegressionDataset(data_tmp2[:n_target_train, :], data_tmp3[:n_target_train]), batch_size=n_target_train, shuffle=True)
    self.dataloader_target_valid = DataLoader(RegressionDataset(data_tmp2[n_target_train:(n_target_train+n_target_valid), :], data_tmp3[n_target_train:(n_target_train+n_target_valid)]), batch_size=n_target_valid, shuffle=True)
    if n_target_test > 0: 
      self.dataloader_target_test = DataLoader(RegressionDataset(data_tmp2[(n_target_train+n_target_valid):, :], data_tmp3[(n_target_train+n_target_valid):]), batch_size=n_target_test, shuffle=True)
    else:
    ...
    self.data_target = {
      ...
      # valid
      'feature_valid': data_tmp2[n_target_train:(n_target_train+n_target_valid), :],
      'feature_valid_spline': [],
      'label_valid': data_tmp3[n_target_train:(n_target_train+n_target_valid)],
      # test
    ...
    for i in range(len(idx_source)):
      ...
      dataloader_train = DataLoader(RegressionDataset(data_tmp2[:n_source_train[i], :], data_tmp3[:n_source_train[i]]), batch_size=n_source_train[i], shuffle=True)
      dataloader_valid = DataLoader(RegressionDataset(data_tmp2[n_source_train[i]:, :], data_tmp3[n_source_train[i]:]), batch_size=n_source_valid[i], shuffle=True)
      self.dataloaders_source_train.append(dataloader_train)
      self.dataloaders_source_valid.append(dataloader_valid)
  ...
  def fetch_credit(self, idx_target = 0):
    ...
    """
    data_raw = pd.read_csv('/Users/huihang/Library/CloudStorage/OneDrive-Personal/Repository/Generative_model/CGAN/CGAN-python/real_data/credit_card/cc.csv', sep=',')
    data_raw = data_raw.sample(frac=1.)
    # print(data_raw.iloc[0:5, :])

    # Filter and clean the data
    tmp_data_clean = {}
    tmp_data_clean['cc10k'] = data_raw[data_raw['X1'] == 10000].drop(columns=['X0', 'X1', 'X11'])
    tmp_data_clean['cc110k'] = data_raw[data_raw['X1'] == 110000].drop(columns=['X0', 'X1', 'X11'])
    tmp_data_clean['cc210k'] = data_raw[data_raw['X1'] == 210000].drop(columns=['X0', 'X1', 'X11'])
    tmp_data_clean['cc310k'] = data_raw[data_raw['X1'] == 310000].drop(columns=['X0', 'X1', 'X11'])
    tmp_data_clean['cc260k'] = data_raw[data_raw['X1'] == 410000].drop(columns=['X0', 'X1', 'X11'])
    tmp_data_clean['cc90k'] = data_raw[data_raw['X1'] >= 510000].drop(columns=['X0', 'X1', 'X11'])
    ...
    def cleanccdata(ccdata):
      ...
      for col in ['X6', 'X7', 'X8', 'X9', 'X10']:
        ...
        ccdata[col] = np.where(ccdata[x_col] == 0, 1, ccdata[f'X{int(col[1:]) + 7}'] / ccdata[x_col])
    ...
    data_tmp2 = np.array(data_tmp1.loc[:, x_cols+z_cols])
    data_tmp3 = np.array(data_tmp1.loc[:, y_col]).reshape(-1, 1)
    dataloader_train = DataLoader(RegressionDataset(data_tmp2[:n_target_train, :], data_tmp3[:n_target_train]), batch_size=n_target_train, shuffle=True)
    ...
    dataloader_valid = DataLoader(RegressionDataset(data_tmp2[n_target_train:(n_target_train+n_target_valid), :], data_tmp3[n_target_train:(n_target_train+n_target_valid)]), batch_size=n_target_valid, shuffle=True)
    if self.flag_test: 
      dataloader_test = DataLoader(RegressionDataset(data_tmp2[(n_target_train+n_target_valid):, :], data_tmp3[(n_target_train+n_target_valid):]), batch_size=n_target_test, shuffle=True)
    else:
    ...
    self.data_target = {
      ...
      'label_train': data_tmp3[:n_target_train],
      # valid
      'feature_valid': data_tmp2[n_target_train:(n_target_train+n_target_test), :],
      'feature_valid_spline': [],
  ...
  def fetch_house(self, idx_target = 0):
    ...
    """
    data_raw = pd.read_csv('/Users/huihang/Library/CloudStorage/OneDrive-Personal/Repository/Generative_model/CGAN/CGAN-python/real_data/house_rent/house_rent.csv', sep=',')
    # rename the '区' in '上海', (+'a'), to avoid the same name with '深圳'
    data_raw.loc[data_raw['城市'] == '上海', '区'] = data_raw.loc[data_raw['城市'] == '上海', '区'].apply(lambda x: x + 'a')
    data_raw = data_raw[['城市', '区', '价格', '室', '卫', '厅', '面积', '所属楼层', '总楼层', '周边学校个数', '周边医院个数', '是否有床', '是否有衣柜', '是否有空调', '是否有燃气']]
    # data_raw = data_raw[data_raw['城市'] == '上海'] # select the target city
    # log transform the price
    # data_raw['价格'] = np.log(data_raw['价格'])
  
    T = len(data_raw['区'].unique()) - 1 # we use different districts as different source domains and set one district as the target domain
    self.T = T
    district_list = data_raw['区'].unique()
    if self.verbose: print(f"district_list: {district_list}")
    district_list = [str(district_list[i]) for i in range(len(district_list))]
    sample_size = [len(data_raw[data_raw['区'] == district_list[i]]) for i in range(self.T+1)]

    y_col = '价格'
    x_cols = ['面积', '周边学校个数']
    z_cols = ['室', '卫', '厅', '所属楼层', '总楼层', '周边医院个数', '是否有床', '是否有衣柜', '是否有空调', '是否有燃气']
    self.dim_linear = len(x_cols)
    self.dim_nonlinear = len(z_cols)
    ...
    print(f"the std of price/area: {np.std(data_raw['价格'])/np.std(data_raw['面积'])}")
    print(f"the std of price/numschool: {np.std(data_raw['价格'])/np.std(data_raw['周边学校个数'])}")
    for col in ['价格', '面积', '周边学校个数', '所属楼层', '总楼层', '周边医院个数']: # ['价格', '面积', '周边学校个数']
      data_raw[col] = (data_raw[col] - np.mean(data_raw[col])) / np.std(data_raw[col])
    # standardize the continuous variables for each task '区' separately
    for col in ['价格']:
      # standardize the continuous variables for each task '区' separately
      for i in range(self.T+1):
        # set the mean of col in the each domain as 0
        data_raw.loc[data_raw['区'] == district_list[i], col] = (data_raw.loc[data_raw['区'] == district_list[i], col] - np.mean(data_raw.loc[data_raw['区'] == district_list[i], col])) # / np.std(data_raw.loc[data_raw['区'] == district_list[i], col])

    # set idx_target as the domain '通州', '虹口a', '龙岗'; 
    # stl 的表现比 mtl 更好: '通州', '海淀'
    # 合适的: '普陀a'
    idx_target = district_list.index('奉贤a')
    print(f"target domain: {district_list[idx_target]} with sample size: {sample_size[idx_target]}")
    # target data
    if self.flag_test:
      n_target_test = int(sample_size[idx_target] * 0.33)
      n_target_train = int(np.min([sample_size[idx_target] * 0.33, 150])) # 200
    else:
      n_target_test = 0
    ...
    n_source_train.pop(idx_target)
    n_source_valid.pop(idx_target)
    if self.verbose: print(f"[RealData]training sample size: {n_source_train}")
  ...
  def generate_spline(self, n_knots=11, data=None):
    ...
    if data is None:
      ...
      z_spline = transform_multidimensional_X(z, common_knots, degree)
      all_feature_train_spline = np.concatenate((self.data_target['feature_train'][:, :self.dim_linear], z_spline), axis=1)

      z = self.data_target['feature_valid'][:, self.dim_linear:]
      z_spline = transform_multidimensional_X(z, common_knots, degree)
      all_feature_valid_spline = np.concatenate((self.data_target['feature_valid'][:, :self.dim_linear], z_spline), axis=1)

      z = self.data_target['feature_test'][:, self.dim_linear:]
      z_spline = transform_multidimensional_X(z, common_knots, degree)
      all_feature_test_spline = np.concatenate((self.data_target['feature_test'][:, :self.dim_linear], z_spline), axis=1)
      return all_feature_train_spline, all_feature_valid_spline, all_feature_test_spline
    else:
      z = data[:, self.dim_linear:]
      z_spline = transform_multidimensional_X(z, common_knots, degree)
      all_feature_spline = np.concatenate((data[:, :self.dim_linear], z_spline), axis=1)
  ...
  def tune_knots(self):
    ...
    loss_val_list = []
    for n_knots in n_knots_list:
      feature_train_spline, feature_valid_spline, freature_test_spline = self.generate_spline(n_knots=n_knots)
      # use ridge spline method to fit the generated data and check the validation loss
      Y = self.data_target['label_train']

      lm = LinearRegression(fit_intercept=False) # RidgeCV(cv=5, fit_intercept=False)
      lm.fit(feature_train_spline, Y)
      tmp_valid_loss = np.mean((lm.predict(feature_valid_spline) - self.data_target['label_valid'])**2)
      loss_val_list.append(tmp_valid_loss)
    ...
    self.data_target['feature_train_spline'], self.data_target['feature_valid_spline'], self.data_target['feature_test_spline'] = self.generate_spline(n_knots=best_n_knots)

    # generate the spline feature for source data
    for i in range(self.T):
      tmp = self.generate_spline(n_knots=best_n_knots, data=self.data_source['feature_train'][i])
      self.data_source['feature_train_spline'].append(tmp)

      tmp = self.generate_spline(n_knots=best_n_knots, data=self.data_source['feature_valid'][i])
  ...
  def clean_data(self):
    ...
    for i, data_train in enumerate(self.data_source['feature_train']):
      if data_train.shape[0] > 200:
        tmp_data_source['feature_train'].append(self.data_source['feature_train'][i])
        tmp_data_source['label_train'].append(self.data_source['label_train'][i])
        tmp_data_source['feature_valid'].append(self.data_source['feature_valid'][i])
        tmp_data_source['label_valid'].append(self.data_source['label_valid'][i])
        tmp_dataloader_train.append(DataLoader(RegressionDataset(self.data_source['feature_train'][i], self.data_source['label_train'][i]), batch_size=self.data_source['feature_train'][i].shape[0], shuffle=True))
        tmp_dataloader_valid.append(DataLoader(RegressionDataset(self.data_source['feature_valid'][i], self.data_source['label_valid'][i]), batch_size=self.data_source['feature_valid'][i].shape[0], shuffle=True))
    ...
    print(f"sample size of each domain: {[self.data_source['feature_train'][i].shape[0] for i in range(self.T)]}")
