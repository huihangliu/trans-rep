...
parser.add_argument(
    "--n_source", help="number of samples in training source", type=int, default=2000
)
parser.add_argument(
...
def joint_train(model_names):
    ...
    if "mtl-pl-2lr" in model_names:
        ...
        if flag_target_train == "default":
            ...
            train_loss = models["mtl-pl-2lr"]["target-nn"].fine_tune(
                dgps.dataloader_target_train
            )
...
if False:
    R = np.concatenate(
        [dgps.params["gammas"][i].reshape((-1, 1)) for i in range(args.T)], axis=1
    )
    # print(f"R: {R}")
...
for i in range(args.r):
    # the estimated one
    plt.plot(
        X_new[:, i], rep_trans[:, i], color=color_list[i], linestyle="-", linewidth=2
    )
    # the true one
    plt.plot(
        X_new[:, i],
        dgps.func_zoo[dgps.params["func_idx"][i]](X_new[:, i]),
        color=color_list[i],
        linestyle="--",
        linewidth=2,
    )
