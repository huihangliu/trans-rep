...
parser.add_argument(
    "--r_dgp", help="representation dimension in dgp", type=int, default=5
)
    ...
    for batch_idx in range(len(dataloaders_aux[0])):
        ...
        for t in range(T):
            if t == 0:
                continue
            loss_sum_aux[t] = (
                loss_sum_aux[t] / loss_sum_aux[t].detach() * loss_sum_aux[0].detach()
            )
        loss_sum_aux = sum(loss_sum_aux)
...
def joint_train(model_names):
    ...
    if "lasso" in model_names:
        ...
        test_loss = np.mean(
            (
                lasso.predict(dgps.data_target["feature_test"])
    ...
    if "ols-linear" in model_names:
        ...
        est_beta_ols = np.linalg.lstsq(
            X_train_linear, dgps.data_target["label_train"], rcond=None
        )[0][:, 0]
        test_loss = np.mean(
            (X_test_linear @ est_beta_ols - dgps.data_target["label_test"][:, 0]) ** 2
        )
        ...
        if args.verbose:
            print(
                model_color["ols-linear"] + f"Model [{'ols-linear'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "ols-all" in model_names:
        ...
        est_beta_ols = np.linalg.lstsq(
            dgps.data_target["feature_train"],
            dgps.data_target["label_train"],
            rcond=None,
        )[0][:, 0]
        test_loss = np.mean(
            (
                dgps.data_target["feature_test"] @ est_beta_ols
                - dgps.data_target["label_test"][:, 0]
            )
            ** 2
        )
        test_perf["ols-all"] = test_loss
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - est_beta_ols[: dgps.dim_linear]
        )
        est_perf["ols-all"] = est_error
        if args.verbose:
            print(
                model_color["ols-all"] + f"Model [{'ols-all'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "ols-oracle" in model_names:
        ...
        X_train_tmp = np.concatenate(
            (X_train_linear, dgps.h(X_train_nonlinear)), axis=1
        )
        est_beta_gamma_ols = np.linalg.lstsq(
            X_train_tmp, dgps.data_target["label_train"], rcond=None
        )[0][:, 0]
        X_test_linear = dgps.data_target["feature_test"][:, : dgps.dim_linear]
        ...
        test_loss = np.mean(
            (X_test_tmp @ est_beta_gamma_ols - dgps.data_target["label_test"][:, 0])
            ** 2
        )
        test_perf["ols-oracle"] = test_loss
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - est_beta_gamma_ols[: dgps.dim_linear]
        )
        est_perf["ols-oracle"] = est_error
        # print(f"estimated value: {est_beta_ols}, true value: {dgps.params['beta'][:,0]}")
        if args.verbose:
            print(
                model_color["ols-oracle"] + f"Model [{'ols-oracle'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )

        test_loss = np.mean(
            (X_test_linear @ dgps.params["beta"] - dgps.data_target["label_test"]) ** 2
        )
    ...
    if "trans-lasso" in model_names:
        if False:
            ...
            X_list = [dgps.data_target["feature_train"][:, : dgps.dim_linear]] + [
                dgps.data_source["feature_train"][t][:, : dgps.dim_linear]
                for t in range(T)
            ]
            Z_list = [dgps.data_target["feature_train"][:, dgps.dim_linear :]] + [
                dgps.data_source["feature_train"][t][:, dgps.dim_linear :]
                for t in range(T)
            ]
            ...
            est_beta_gamma = np.concatenate(
                (res_trans_lasso["beta_hat"], res_trans_lasso["alpha_hat"])
            )
        else:
            X_list = [dgps.data_target["feature_train"][:, : dgps.dim_linear]] + [
                dgps.data_source["feature_train"][t][:, : dgps.dim_linear]
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - est_beta_gamma[: dgps.dim_linear]
        )
        est_perf["trans-lasso"] = est_error
        if args.verbose:
            print(
                model_color["trans-lasso"] + f"Model [{'trans-lasso'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "meta-analysis" in model_names:
        ...
        X_list = [dgps.data_target["feature_train"]] + [
            dgps.data_source["feature_train"][t] for t in range(args.T)
        ]
        Z_list = [dgps.data_target["feature_train"][:, :0]] + [
            dgps.data_source["feature_train"][t][:, :0] for t in range(args.T)
        ]
        Y_list = [dgps.data_target["label_train"]] + dgps.data_source["label_train"]
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - res_meta_analysis["beta_hat"][: dgps.dim_linear]
        )
        est_perf["meta-analysis"] = est_error
        if args.verbose:
            print(
                model_color["meta-analysis"] + f"Model [{'meta-analysis'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "pooled-regression" in model_names:
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - res_pooled_regression["beta_hat"]
        )
        est_perf["pooled-regression"] = est_error
        if args.verbose:
            print(
                model_color["pooled-regression"]
                + f"Model [{'pooled-regression'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "map" in model_names:
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - res_map["beta_hat"][: dgps.dim_linear]
        )
        est_perf["map"] = est_error
        if args.verbose:
            print(
                model_color["map"] + f"Model [{'map'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "trans-lasso-spline" in model_names:
        if False:  # use 'ma' implementation
            ...
            Z_list = [
                dgps.data_target["feature_train_spline"][:, dgps.dim_linear :]
            ] + [
        ...
        if args.verbose:
            print(
                model_color["trans-lasso-spline"]
                + f"Model [{'trans-lasso-spline'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "trans-ridge-spline" in model_names:
        ...
        est_beta_gamma_trans_ridge = np.concatenate(
            (res_trans_ridge["beta_hat"], res_trans_ridge["alpha_hat"])
        )

        test_loss = np.mean(
            (
                dgps.data_target["feature_test_spline"] @ est_beta_gamma_trans_ridge
                - dgps.data_target["label_test"][:, 0]
            )
            ** 2
        )
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - res_trans_ridge["beta_hat"]
        )
        est_perf["trans-ridge-spline"] = est_error
        if args.verbose:
            print(
                model_color["trans-ridge-spline"]
                + f"Model [{'trans-ridge-spline'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "meta-analysis-spline" in model_names:
        ...
        X_list = [dgps.data_target["feature_train_spline"]] + [
            dgps.data_source["feature_train_spline"][t] for t in range(args.T)
        ]
        Z_list = [dgps.data_target["feature_train_spline"][:, :0]] + [
            dgps.data_source["feature_train_spline"][t][:, :0] for t in range(args.T)
        ]
        Y_list = [dgps.data_target["label_train"]] + dgps.data_source["label_train"]
        res_meta_analysis = model_averaging.meta_analysis(Y_list, X_list, Z_list)
        est_beta_gamma = np.concatenate(
        ...
        if args.verbose:
            print(
                model_color["meta-analysis-spline"]
                + f"Model [{'meta-analysis-spline'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "pooled-regression-spline" in model_names:
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0]
            - res_pooled_regression["beta_hat"][: dgps.dim_linear]
        )
        est_perf["pooled-regression-spline"] = est_error
        if args.verbose:
            print(
                model_color["pooled-regression-spline"]
                + f"Model [{'pooled-regression-spline'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "map-spline" in model_names:
        ...
        if args.verbose:
            print(
                model_color["map-spline"] + f"Model [{'map-spline'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "ols-spline" in model_names:
        ...
        test_loss = np.mean(
            (
                lm.predict(dgps.data_target["feature_test_spline"])
                - dgps.data_target["label_test"]
            )
            ** 2
        )
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - lm.coef_[0, : dgps.dim_linear]
        )
        est_perf["ols-spline"] = est_error
        if args.verbose:
            print(
                model_color["ols-spline"] + f"Model [{'ols-spline'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "linear-rep" in model_names:
        ...
        est_beta_gamma = np.concatenate(
            (res_linear_rep["beta_hat"], res_linear_rep["alpha_hat"])
        )
        ...
        est_error = np.linalg.norm(
            dgps.params["beta"][:, 0] - res_linear_rep["beta_hat"]
        )
        est_perf["linear-rep"] = est_error
        if args.verbose:
            print(
                model_color["linear-rep"] + f"Model [{'linear-rep'}] report: "
                f"current test loss = {test_loss}, "
                f"est error = {est_error}"
            )
    ...
    if "mtl-pl-2lr" in model_names:
        ...
        for epoch in range(num_epochs_pretrain):
            ...
            if (
                True
            ):  # old update method: update the shared network when the source models have better valid loss
                    ...
                    if valid_loss_target < 1.05 * best_valid_target:
                        ...
                        if True and args.verbose:
                            print(
                                ...
                                + f"Model [{'mtl-pl-2lr'}] source update test loss, "
                        ...
                        if args.verbose:
                            print(
                                Fore.WHITE + f"Model [{'mtl-pl-2lr'}] source,"
        ...
        if False and args.verbose:
            print(f"estimated variance: {est_var}")
    ...
    for epoch in range(num_epochs):
        ...
        for model_name in model_names:
            if last_update_count[model_name] > patience:
                continue
            ...
            if valid_loss < best_valid[model_name]:
                ...
                if args.verbose:
                    print(
                        ...
                        + f"Model [{model_name}] update test loss, "
                        f"best valid loss = {valid_loss}, current test loss = {test_loss}, "
                        f"est error = {est_error}"
                    )
...
if True:
    ...
    if args.verbose:
        print(f"\tthe seleted best n_knots: {best_n_knots}")
    # sample from the dgp
...
vanilla_nn_model_orgigin = RegressionNN(
    d=args.p + args.q, depth=args.depth, width=args.width
)
vanilla_nn_model = PartialRegressionNN(
    dim_input=args.p + args.q,
    dim_linear=args.q,
    depth=args.depth,
    width=args.width,
...
stl_1lr_model = PartialLinearRegression(
    dim_input=args.p + args.q, dim_linear=args.q, depth=args.depth, width=args.width
)
...
dropout_stl_2lr_model = PartialLinearRegression(
    dim_input=args.p + args.q,
    dim_linear=args.q,
    dim_rep=args.r,
    depth=args.depth,
...
for method_name, model_x in models.items():
    ...
    if method_name in [
        ...
        optimizer_source_linear = [
            torch.optim.Adam(nn_model.linear_output.parameters(), learning_rate_linear)
            for nn_model in model_x["aux-nn"]
        ]
...
model_names = [
    ...
    "trans-lasso-spline",
    "map-spline",
    "meta-analysis-spline",
    "pooled-regression-spline",
    "stl-pl-2lr",
    "ols-linear",
]
...
test_l2_error = np.concatenate(
    (test_l2_error, np.array([args.seed]).reshape(1, 1)), axis=1
)
est_error = np.concatenate((est_error, np.array([args.seed]).reshape(1, 1)), axis=1)
if len(args.record_dir) > 0:
    res_pred_file = open(
        args.record_dir
        + f"/exp{args.exp_id}_n{args.n_source}_r{args.r}_p{args.p}_pred.csv",
        "a",
    )
    res_est_file = open(
        args.record_dir
